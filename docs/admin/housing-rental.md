...
物件名・家賃・WorldGuard領域の作成・親リージョン継承・フラグ設定まで
すべて自動で行われます。**通常はこれだけで足ります。**

1. 範囲選択ツール（木の斧）を受け取る（最初の1回だけ）
   ```bash
   /housing admin wand
   ```

2. 部屋の角を**左クリック**（第1座標）、対角を**右クリック**（第2座標）
   選択範囲が緑のパーティクルで表示されます
...
3. クイック登録
   ...
   ```bash
   # 物件名・家賃を指定したい場合
   /housing admin qr --name "中央区マンション101号室" --rent 150
   ```

自動で設定される内容:
- **物件名**: `house-1`, `house-2`, … と自動採番（`auto_naming.prefix` で変更可）
- **日額賃料**: `housing_rental.default_daily_rent`（既定100）
- **週額・月額**: 日額 × 6.5 / × 25 で自動算出
- **WorldGuard領域**: 物件名と同名で自動作成、親リージョンとフラグも自動設定
- **ドア・チェストの権限**: 賃貸契約時に自動付与、契約終了時に自動剥奪

#### GUI登録

チャット入力で物件名・家賃を指定し、他リージョンとの重なりも確認できます。

```bash
/housing admin registergui
```
（`/housing admin gui` でも可）
...
```bash
/housing admin manage
```

#### 従来の登録コマンド

物件名と日額賃料を毎回指定する方式です。クイック登録で十分な場合は不要です。

```bash
/housing admin register <物件名> <日額賃料>

# WorldGuard領域名を明示したい場合
