...
当初計画は完了扱いだが、以下は実装が伴っていない。プレイヤー向け
ドキュメントで約束しないよう注意すること。

| 項目 | 状態 |
|---|---|
| 職業スキル（全24種） | `config/jobs.yml` に定義とDBシードはあるが、効果を発動するコードが無い |
| Lv40以降のレベル報酬 | 売却価格へのレベル反映が無く、5レベルごとの一時金のみ |
| 上級職・鍛冶屋のクエスト | `JobQuestManager` のクエストは5個のみ、いずれも Lv1〜5 |
| 職業収入システム | `JobIncomeManager` は無効化済み（収入はNPC売却とマーケットのみ） |
| 賃貸の期限前通知 | 未実装 |
